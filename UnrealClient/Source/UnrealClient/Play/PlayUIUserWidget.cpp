...
void UPlayUIUserWidget::PostWindowOpen()
{
	UClientGameInstance* Inst = Cast<UClientGameInstance>(UGameplayStatics::GetGameInstance(GetWorld()));
	...
	UPlayUIUserWidget::PostWindowOnOff();
}

void UPlayUIUserWidget::PostWindowOnOff()
{
	UClientGameInstance* Inst = Cast<UClientGameInstance>(UGameplayStatics::GetGameInstance(GetWorld()));
	...
	switch (Inst->PostWindowMode)
	{
	case ESlateVisibility::Visible:
		Inst->PostWindowMode = ESlateVisibility::Hidden;
		break;
	case ESlateVisibility::Hidden:
		Inst->PostWindowMode = ESlateVisibility::Visible;
		break;
	case ESlateVisibility::Collapsed:
	case ESlateVisibility::HitTestInvisible:
	case ESlateVisibility::SelfHitTestInvisible:
	...
	Inst->PostWindow->SetVisibility(Inst->PostWindowMode);
}
...
void UPlayUIUserWidget::NativeConstruct() 
{
	...
	Inst->PostWindow = GetWidgetFromName(TEXT("PostWindow"));
	...
	if (nullptr == Inst->PostWindow ||
		false == Inst->PostWindow->IsValidLowLevel())
	{
		UE_LOG(ClientLog, Error, TEXT("Message List View Is Nullptr"));
		return;
	}

...
// 레퍼런스 카운트는 항상 1인 상태야.
ThreadHandlerUserPostMessage::ThreadHandlerUserPostMessage()
{
...
void ThreadHandlerUserPostMessage::Start()
{
	...
	DBWork(&ThreadHandlerUserPostMessage::DBCheck);
}

void ThreadHandlerUserPostMessage::DBCheck(/*GameEngineThread* _Thread*/)
{
	...
	if (MyRank.Rank == -1)
	{
		NetWork(&ThreadHandlerUserPostMessage::ResultSend);
		return;
	}
...
void ThreadHandlerUserPostMessage::ResultSend()
{
	/*
